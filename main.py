...
# --- Regional supplies (prices + store types near selected lat/lon) ---
...
def _infer_money_region(lat: float, lon: float) -> tuple[str, str, str]:
...
def _mk_price_estimate(name: str, currency: str, symbol: str) -> dict[str, Any]:
    seed = _js_string_hash(name + currency)
    is_inr = currency == "INR"
    if is_inr:
        low = 180 + (seed % 4200)
...
def _mk_stores(lat: float, lon: float, region_label: str) -> list[dict[str, Any]]:
    seed = _js_string_hash(f"{lat:.2f}|{lon:.2f}")
    if _infer_money_region(lat, lon)[0] == "INR":
        base = [
...
def _build_regional_supplies(
    lat: float,
    lon: float,
    location_label: str,
    treatments: list[dict[str, Any]],
) -> dict[str, Any]:
    currency, symbol, region_hint = _infer_money_region(lat, lon)
    region_label = (
    ...
    price_estimates = [_mk_price_estimate(n, currency, symbol) for n in names]
    stores = _mk_stores(lat, lon, region_label)
    disclaimer = (
...
class RegionalSuppliesBody(BaseModel):
    lat: float = Field(..., ge=-90, le=90)
    lon: float = Field(..., ge=-180, le=180)
    location_label: str = ""
    treatments: list[TreatmentIn] = Field(default_factory=list)


@app.post("/regional-supplies")
def regional_supplies(body: RegionalSuppliesBody):
    """
    Typical local price bands + example ag retail channels for the treatments list,
    keyed off the same lat/lon the app uses on the Weather tab.
    """
    items = [t.model_dump() for t in body.treatments]
    if not items:
        return {
    ...
    return _build_regional_supplies(body.lat, body.lon, body.location_label, items)
